...
- ⭐ **价值评估**：自动识别高价值论文（评分>3.0），并单独导出到 CSV
- 📊 **CSV 导出**：高价值论文和所有相关论文自动导出为 CSV 表格，方便管理和追踪
- 💾 **备份支持**：支持将报告自动备份到指定目录
...
同时会生成 CSV 文件：
```
高价值论文_YYYYMMDD.csv          # 高价值论文列表（仅包含标题和链接）
相关论文_YYYYMMDD.csv              # 所有相关论文列表（包含标题、链接、摘要和处理结果）
```

CSV 文件说明：
- **高价值论文 CSV**：包含论文标题和链接，方便快速查看和下载
- **相关论文 CSV**：包含论文标题、链接、摘要（如果提取成功）和处理结果（1=成功，0=失败）
